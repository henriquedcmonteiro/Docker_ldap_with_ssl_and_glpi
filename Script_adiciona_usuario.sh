...
if ldapsearch -x -H "$LDAP_SERVER" -D "$LDAP_ADMIN_DN" -w "$LDAP_ADMIN_PASS" -b "$BASE_DN" "uid=$USERNAME" | grep -q "dn:"; then
    error_exit "Erro: O nome de usuário '$USERNAME' já existe no LDAP."
fi

# Buscar o próximo uidNumber disponível
LAST_UID=$(ldapsearch -x -H "$LDAP_SERVER" -D "$LDAP_ADMIN_DN" -w "$LDAP_ADMIN_PASS" -b "$BASE_DN" "(uidNumber=*)" uidNumber | awk '/uidNumber:/ {print $2}' | sort -n | tail -1)
if [[ -z "$LAST_UID" ]]; then
