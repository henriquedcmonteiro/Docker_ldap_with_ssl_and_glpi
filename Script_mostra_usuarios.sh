...
ldapsearch -x -H "LDAP_SERVER" -D "$LDAP_ADMIN_DN" -w "$LDAP_ADMIN_PASS" -b "$BASE_DN" "(objectClass=posixAccount)" uid uidNumber | grep -E "uid:|uidNumber:" | paste - - | awk '{print $4 "\t" $2}'
