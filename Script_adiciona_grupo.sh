...
if ldapsearch -x -H "$LDAP_SERVER" -D "$LDAP_ADMIN_DN" -w "$LDAP_ADMIN_PASS" -b "$GROUP_BASE_DN" "cn=$GROUPNAME" | grep -q "dn:"; then
    error_exit "Erro: O grupo '$GROUPNAME' já existe no LDAP."
fi

# Buscar o próximo gidNumber disponível
LAST_GID=$(ldapsearch -x -H "$LDAP_SERVER" -D "$LDAP_ADMIN_DN" -w "$LDAP_ADMIN_PASS" -b "$GROUP_BASE_DN" "(gidNumber=*)" gidNumber | awk '/gidNumber:/ {print $2}' | sort -n | tail -1)
if [[ -z "$LAST_GID" ]]; then
...
read -p "Digite o gidNumber (ou pressione Enter para usar $NEXT_GID): " GIDNUMBER
if [[ -z "$GIDNUMBER" ]]; then
    GIDNUMBER=$NEXT_GID
fi
...
if ldapsearch -x -H "$LDAP_SERVER" -D "$LDAP_ADMIN_DN" -w "$LDAP_ADMIN_PASS" -b "$GROUP_BASE_DN" "gidNumber=$GIDNUMBER" | grep -q "gidNumber:"; then
    error_exit "Erro: O gidNumber '$GIDNUMBER' já está em uso por outro grupo no LDAP."
fi
